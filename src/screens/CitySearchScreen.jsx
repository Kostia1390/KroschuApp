...
const CitySearchScreen = ({ navigation }) => {
  ...
  const sendRequestToServer = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://morning-escarpment-40202.herokuapp.com/process_image",
        {
          method: "POST",
          body: formData,
          headers: {
  ...
  return (
    <LinearGradient colors={["#a7ddef", "#3aa1c9"]} style={styles.container}>
      ...
      {image && (
        ...
        >
          <Text style={{ fontSize: 22, fontWeight: "semibold" }}>
      ...
      <TouchableOpacity style={styles.acceptButtonWrapper} onPress={pickImage}>
        <Text style={{ fontSize: 22, fontWeight: "semibold" }}>
          Оберіть фото з галереї
        </Text>
      </TouchableOpacity>
      {requestStatus && (
        <Text
          style={{ fontSize: 22, fontWeight: "semibold", marginVertical: 20 }}
        >
          {requestStatus}
        </Text>
      )}
